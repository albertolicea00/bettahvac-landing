<script>
  ...
  const handleSubmit = async () => {
    ...
    if (result && result.success) {
      ...
      const tempTotal = reviewsData.averageRating * (reviewsData.totalReviews - 1) + newReview.rating;
      reviewsData.averageRating = (tempTotal / reviewsData.totalReviews).toFixed(1);
...
<section id="reviews" class="section section-alt">
  <div class="container">
    <div class="reviews-header">
      <div>
        <h2 class="section-title" style="margin-bottom: 0.5rem;">Customer Reviews</h2>
        {#if !loading}
      ...
      <div class="review-form-container glass-panel">
        ...
        <form on:submit|preventDefault={handleSubmit}>
          <div class="form-group">
            ...
            <input id="author" type="text" bind:value={newReview.author} required placeholder="John Doe">
          </div>
          <div class="form-group">
            <label for="rating">Rating (1-5)</label>
            <input id="rating" type="number" min="1" max="5" bind:value={newReview.rating} required>
          </div>
          <div class="form-group">
            <label for="text">Review Content</label>
            <textarea id="text" bind:value={newReview.text} required rows="3" placeholder="Share details of your own experience at this place"></textarea>
          </div>
    ...
    <!-- Reviews Grid -->
    {#if loading}
      <p style="text-align: center; color: var(--color-text-light);">Loading reviews...</p>
    {:else}
